function LeapLogger () {
  ...
  function outputDebugInfo (el, hands){
    ...
    if(hands['l']){
      outputContent += '<br> Are we grabbing shit with our lefthand?' + hands['l'].grabStrength;
      outputContent += '<br> Are we pinching shit?' + hands['l'].pinchStrength;
      outputContent += '<br> They see me rolling left'   + hands['l'].roll();
    }
    if(hands['r']) {
      outputContent += '<br> Are we grabbing shit with our righthand?' + hands['r'].grabStrength;
      outputContent += '<br> Are we pinching shit?' + hands['r'].pinchStrength;
      outputContent += '<br> They see me rolling right'   + hands['r'].roll();
    }
  };
